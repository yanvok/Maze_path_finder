...
void func(m &m){
    ...
    srand(static_cast<unsigned>(time(0))); //randm generator

    int sx = m.Sx, sy = m.Sy, ex = m.Ex, ey = m.Ey;
    vector<pair<int,int>> v; //potential cells
    ...
    while(!v.empty()) {
        ...
        int randIndex = rand() % v.size(); //pick idx 
        auto [cx, cy] = v[randIndex];

        v[randIndex] = v.back(); //swap with last and rm
        v.pop_back(); 
        count++;
