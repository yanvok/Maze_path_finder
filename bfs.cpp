...
Path pathRec(const pos &end, const pos &st,const vector<vector<pair<int,int>>> &parent);
void print(const m & m, const vector<vector<bool>> & visited,bool finalPath = false, const Path & P = {});
...
void func(m &m){
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++){
            ...
            if (empty(m, ny, nx) && !visited[ny][nx]){
                visited[ny][nx] = true;
                parent[ny][nx] = {cx, cy};
                q.push({nx, ny});
            }
    ...
    Path P;
    if (foundEnd) {
        P = pathRec(st, end, parent);
    }

    print(m, visited, true, P);
    cout << "Nodes expanded: " << count << "\n";
    if(!foundEnd){
        cout << "No path\n";
    }
    else cout << "Path length: " << (P.size() - 1) << "\n";
